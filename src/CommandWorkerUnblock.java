...
public class CommandWorkerUnblock extends CommandWorkerBase implements ICommandWorker {
    ...
    public boolean unblockUser(String user) {
        ...
        String[] results = utility.searchFile(
                utility.concatStrings(square.getSafeLowerName(), Constants.MEMBERS_FILE_EXT), user,
                Constants.SEARCH_STARTS_WITH);
        if (results.length > 0) {
            String[] memberInfo = results[0].split(Constants.FILE_DATA_SEPARATOR);
            String[] blockedMembers = utility
                    .readFile(utility.concatStrings(square.getSafeLowerName(), Constants.BLOCK_FILE_EXT))
            ...
            for (String blockedMember : blockedMembers) {
                if (!blockedMember.equals(memberInfo[4])) {
                    output.append(utility.concatStrings(newline, blockedMember));
                }
                if (first) {
                    first = false;
            ...
            FileWriteResponse response = utility.writeFile(
                    utility.concatStrings(square.getSafeLowerName(), Constants.BLOCK_FILE_EXT), output.toString());
